...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(
			...
			sqlmock.AnyArg(), // parent_account_id
			log.InputTokens,
			log.OutputTokens,
...
func TestUsageLogRepositoryGetUserTokenRanking(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "username", "actual_cost", "requests", "tokens", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(7), "seven@example.com", "Seven", 1.5, int64(3), int64(7000), 2.0, int64(5), int64(8000))

	mock.ExpectQuery("WITH user_tokens AS \\(").
		WithArgs(start, end, 10).
		WillReturnRows(rows)

	got, err := repo.GetUserTokenRanking(context.Background(), start, end, 10)
	require.NoError(t, err)
	require.Equal(t, []usagestats.UserSpendingRankingItem{{UserID: 7, Email: "seven@example.com", Username: "Seven", ActualCost: 1.5, Requests: 3, Tokens: 7000}}, got.Ranking)
	require.Equal(t, int64(8000), got.TotalTokens)
	require.NoError(t, mock.ExpectationsWereMet())
}
