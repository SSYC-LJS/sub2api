...
type GatewayConfig struct {
	...
	// RequestResponseCapture: 请求/响应正文全量采集配置。
...
type GatewayRequestResponseCaptureConfig struct {
	...
	Enabled bool `mapstructure:"enabled"`
	// GroupID: 仅采集指定分组；0 表示全部分组。
	GroupID int64 `mapstructure:"group_id"`
	// MaxBodyBytes: 已弃用。保留该字段仅兼容旧配置，采集不再截断正文。
...
func setDefaults() {
	...
	viper.SetDefault("gateway.request_response_capture.enabled", true)
	viper.SetDefault("gateway.request_response_capture.group_id", 0)
	viper.SetDefault("gateway.request_response_capture.max_body_bytes", 0)
	viper.SetDefault("gateway.usage_record.overflow_sample_percent", 10)

	viper.SetDefault("batch_image.enabled", true)
	viper.SetDefault("batch_image.max_items_per_job_default", 10)
	viper.SetDefault("batch_image.max_items_per_job_trial", 3)
	viper.SetDefault("batch_image.max_output_images_per_job", 40)
	viper.SetDefault("batch_image.max_output_images_per_item", 4)
	viper.SetDefault("batch_image.max_prompt_chars_per_item", 4000)
	viper.SetDefault("batch_image.max_reference_images_per_job", 10)
	viper.SetDefault("batch_image.max_reference_inline_bytes_per_job", int64(20*1024*1024))
	viper.SetDefault("batch_image.default_response_mime_type", "image/png")
	viper.SetDefault("batch_image.default_image_size", "1K")
	viper.SetDefault("batch_image.max_download_items_zip", 100)
	viper.SetDefault("batch_image.max_download_bytes_per_request", int64(512*1024*1024))
	viper.SetDefault("batch_image.max_download_duration_seconds", 60)
	...
	viper.SetDefault("batch_image.cleanup_interval_minutes", 60)
	viper.SetDefault("batch_image.cleanup_batch_size", 100)
	viper.SetDefault("batch_image.queue_enabled", false)
	viper.SetDefault("batch_image.queue_ready_key", "sub2api:batch_image:ready")
	viper.SetDefault("batch_image.queue_delayed_key", "sub2api:batch_image:delayed")
	viper.SetDefault("batch_image.queue_active_key", "sub2api:batch_image:active")
	viper.SetDefault("batch_image.inflight_key_prefix", "sub2api:batch_image:inflight:")
	viper.SetDefault("batch_image.lock_key_prefix", "sub2api:batch_image:lock:")
	viper.SetDefault("batch_image.idempotency_key_prefix", "sub2api:batch_image:idempotency:")
	viper.SetDefault("batch_image.inflight_ttl_seconds", 900)
	viper.SetDefault("batch_image.job_lock_ttl_seconds", 300)
	viper.SetDefault("batch_image.default_requeue_delay_seconds", 5)
	viper.SetDefault("batch_image.error_retry_delay_seconds", 30)
	viper.SetDefault("batch_image.lock_conflict_delay_seconds", 10)
	viper.SetDefault("batch_image.stale_active_after_seconds", 900)
	viper.SetDefault("batch_image.delayed_mover_interval_seconds", 2)
	viper.SetDefault("batch_image.recovery_interval_seconds", 30)
	...
	viper.SetDefault("batch_image.vertex_managed_gcs_prefix", "sub2api-batch-image/{batch_id}")
