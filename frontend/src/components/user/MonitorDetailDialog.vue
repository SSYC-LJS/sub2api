      ...
      <table class="w-full text-left text-sm">
        <thead class="border-b border-gray-200 dark:border-dark-700">
          <tr class="text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400">
            ...
            <th class="py-2 pr-3">{{ t('channelStatus.detailColumns.availability12h') }}</th>
            <th class="py-2 pr-3">{{ t('channelStatus.detailColumns.avgLatency12h') }}</th>
          </tr>
        </thead>
        <tbody>
          <tr
          ...
          >
            ...
            <td class="py-2 pr-3 text-gray-700 dark:text-gray-300">{{ formatPercent(m.availability_12h ?? 100) }}</td>
            <td class="py-2 pr-3 text-gray-700 dark:text-gray-300">{{ formatLatency(m.avg_latency_12h_ms) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
